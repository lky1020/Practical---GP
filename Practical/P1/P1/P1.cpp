...
void circle() {
	//Line Circle: GL_POINTS, GL_LINE_LOOP
	//Field Circle: GL_POLYGON (need pie), GL_TRIANGLE_FAN

	//x = xPoint1 + r * cos(angle)
	//y = yPoint1 + r * sin(angle)
	yPoint1 = 0.375;
	...
	for (int i = 0; i < 360; i++) {
		angle = i * 3.142 / 180;
		glVertex2f(0.20 * cos(angle), 0.25 * sin(angle));
	}
	glEnd();*/
